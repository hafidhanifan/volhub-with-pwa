...
<section class="w-full">
  <div class="py-8 px-12 max-w-5xl">
    ...
    <form action="{{ route('mitra.edit-kegiatan-action', ['id' => $mitra->id_mitra,'id_keg' => $kegiatan->id_kegiatan]) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="grid gap-4 mt-8 sm:grid-cols-2 sm:gap-6">
          ...
          <select name="kategori" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5">
            <option selected="">Select Category</option>
              @foreach($kategori as $kategori) 
                <option value="{{ $kategori->id_kategori }}"@if(old('kategori', $kegiatan->id_kategori) == $kategori->id_kategori) selected @endif>{{ $kategori->nama_kategori }}</option>
              @endforeach
          </select>
        </div>
        <div class="col-span-2 lg:col-span-1">
        ...
        <div class="col-span-2 lg:col-span-1">
          ...
          <input type="date" name="tgl_kegiatan" id="start_date"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
            placeholder="Pilih tanggal kegiatan dimulai" required value="{{ old('kegiatan', $kegiatan->tgl_kegiatan) }}"/>
        </div>
        ...
        <div class="col-span-2 lg:col-span-1">
          ...
          <input type="date" name="tgl_penutupan" id="end_date"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
            placeholder="Pilih tanggal pendaftaran berakhir" required value="{{ old('kegiatan', $kegiatan->tgl_penutupan) }}"/>
        </div>
