...
<body style="background-image: url('{{ asset('img/registration-login-image/registration-login-image.jpg') }}');"
  ...
  <div class="absolute w-full top-0 z-20 flex justify-between px-2 py-4">
    <a href="{{route('home')}}" class="block text-lg font-medium py-1 px-4 text-white">API Employer</a>
    <a href="{{route('api.mitra.login')}}" class="text-lg font-medium text-white px-4 py-1 rounded-lg hover:bg-white hover:text-sky-600">
      Sign In
    </a>
  </div>
  ...
  <div class="relative z-10 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 w-5/6 mx-auto my-auto rounded-lg lg:w-2/6">
      <div class="flex flex-col gap-2 mx-auto max-w-52 md:max-w-64">
        <h1 class="text-center font-semibold md:text-xl">API Register Employer</h1>
        <p class="py-2 text-center text-xs md:text-sm">
          Hey, Enter your details to get Register your account
        </p>
      </div>
      <form action="{{ route('api.mitra.register.action')}}" method="POST">
        @csrf
      ...
      <div class="mt-6 flex justify-center gap-1">
        ...
        <a href="{{route('api.mitra.login')}}" class="text-sky-500 text-xs md:text-sm">Sign In</a>
      </div>
    </div>
  </div>
  <!-- Content end -->
