...
<section class="w-full">
  <div class="p-4">
    <h1 class="font-normal text-lg">Add Benefit</h1>
    <p class="text-gray-500 text-sm">
      Add Benefits according to your needs
    </p>
  </div>
  <div class="p-4 max-w-lg">
    <ul class="divide-y divide-gray-200">
      ...
      <li class="flex justify-between items-center py-2 gap-4">
        ...
        <form action="{{ route('mitra.remove-benefit-action', ['id_keg' => $kegiatan->id_kegiatan, 'id_benefit' => $benefit->id_benefit])}}" method="POST" style="display: inline;">
          @csrf
          @method('DELETE')
  ...
  <form action="{{ route('mitra.add-benefit-kegiatan-action', ['id_keg' => $kegiatan->id_kegiatan]) }}" method="POST">
    @csrf
    <div class="p-4 max-w-lg">
      <label for="" class="block mb-2 text-sm font-medium">Add benefit</label>
      <input type="text" name="nama_benefit" id="" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5"
