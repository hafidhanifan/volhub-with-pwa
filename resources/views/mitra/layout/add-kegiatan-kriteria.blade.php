      ...
      <div class="row">
        <div class="col-lg-6">
          ...
          <div class="card-style mb-30 d-flex flex-column">
                ...
                @foreach ($kegiatan->Kriterias as $kriteria)
                  <div class="button_custom_kegiatan">
                    <li>{{ $kriteria->nama_kriteria }}</li>
                      <form action="{{ route('mitra.remove-kriteria-action', ['id' => $kegiatan->id_kegiatan, 'id_kriteria' => $kriteria->id_kriteria])}}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
            ...
            <form action="{{ route('mitra.add-kriteria-kegiatan-action', ['id' => $mitra->id_mitra,'id_keg' => $kegiatan->id_kegiatan]) }}" method="POST">
              @csrf
