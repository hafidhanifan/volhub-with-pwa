...
<section class="w-full">
  ...
  <div class="p-4 max-w-lg">
    <ul class="divide-y divide-gray-200">
      ...
      <li class="flex justify-between items-center py-2 gap-4">
        ...
        <form action="{{ route('mitra.remove-kriteria-action', ['id_keg' => $kegiatan->id_kegiatan, 'id_kriteria' => $kriteria->id_kriteria])}}" method="POST" style="display: inline;">
          @csrf
          @method('DELETE')
  ...
  <form action="{{ route('mitra.add-kriteria-kegiatan-action', ['id_keg' => $kegiatan->id_kegiatan]) }}" method="POST">
    @csrf
    <div class="p-4 max-w-lg">
      <label for="" class="block mb-2 text-sm font-medium">Add requirement</label>
      <input type="text" name="nama_kriteria" id="" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5"
